...
PLOD %>%
  ...
  pivot_longer(cols = Q7_1:Q7_6,
               names_to = "Question",
               values_to = "Choice") %>%
  mutate(Choice = factor(Choice, levels = c("Not sure", "Strongly disagree", "Disagree", "Neutral", "Agree", "Strongly agree"))) %>%
  group_by(Question) %>% 
  count(Choice) %>%
  ggplot(aes(fill=Choice, values=n)) +
    geom_waffle(color = "white", size=1.125, n_rows = 6) +
    facet_wrap(~Question, ncol=3, labeller=labeller(Question = Q7labels2)) +
